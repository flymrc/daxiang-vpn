...
- `hub/` — **Hub 服务端**(授权 API)。
- `egress/` — **出口节点**(基础设施侧,非终端客户端)。`egress/proxy/` = 跨平台 Go 出口代理(linux-arm64 即安卓 `dxandroid-egress`,darwin/windows 即 Mac/PC 出口,🅿️ 预留);`egress/android-status/` = 安卓出口监控 App;`egress/android-control/` = 安卓出口远程控制+自愈(自研 Go SSH 服务 `dxandroid-control` 绑隧道 IP 10.66.0.101:22、仅公钥 + 看门狗,配套 Hub 探针 `scripts/watch-android-egress-liveness.sh`)。
