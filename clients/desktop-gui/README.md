...
`status()` 在 Rust 后端有全局异步锁，前端也会跳过仍在进行中的刷新；主窗口和托盘同时轮询时不会叠出多个长期停留的 `zhvpn.exe status --json` 子进程。公网出口 IP 由 `statusIp()` 按需/低频刷新，GUI 会同时展示 IPv6 与 IPv4，并保留上一轮有效值，避免未换 IP 时界面反复跳「获取中」。
