...
| # | 问题 | 严重度 | 已验证事实 |
| --- | --- | --- | --- |
| 1 | zhhub 明文 HTTP 暴露公网 | 🔴 高 | 从中国直连 `http://36.50.84.68:18080/healthz` 成功返回 |
...
| 7 | zhhub 以 root 运行 | 🟠 中 | 进程归属 root（pid 22841），无降权用户 |
| 8 | zhhub 无审计日志 / 无限流 | 🟡 低 | 代码层面，bootstrap 无频率限制、无访问审计 |
...
- 放行规则：
  ...
  - `18080/tcp`（zhhub bootstrap，临时保留，待 #1 TLS 上线后收）
...
已上线的 HTTPS 入口是 Hub 管理控制台：Caddy 对公网提供
`https://jp-proxy.ruichao.dev/admin/`，再反代到本机 `127.0.0.1:18100`。
这解决的是 admin 管理面的浏览器访问安全，不等同于本报告 #1/#2。

截至 2026-07-01，客户端授权 API 仍直接使用 `http://36.50.84.68:18080`：
`/api/client/bootstrap` 和 `/api/client/rotate-ip` 仍未迁到 HTTPS。也就是说，P0-1
「客户端只通过 HTTPS 访问 bootstrap」尚未完成；P0-2「客户端私钥本地生成、Hub
不再下发私钥」也尚未完成。

同日后续进展：Caddy 已上线 `/api/client/*` 和 `/healthz` 到 `127.0.0.1:18080`
的 HTTPS 反代,公网 `https://jp-proxy.ruichao.dev/healthz` 返回 ok,
`https://jp-proxy.ruichao.dev/api/client/bootstrap` 对 HEAD 返回 405。P0-1 仍待
新客户端发布和公网 `18080/tcp` 收口后关闭。

同日 P0-2 进展：生产 `zhhub` 已部署支持客户端上报 `wireguard_public_key` 的新协议。
在 Hub 本机用生产已有 token 派生同一公钥验证:bootstrap 返回 `200 OK`,响应包含
`wireguard.public_key`,不再包含 `wireguard.private_key`;`wg0` peer 数保持 25。
P0-2 仍待新客户端分发、GUI/Python SDK bundled CLI 覆盖,以及生产 `tokens.yaml`
清理 legacy `wireguard.private_key` 后关闭。
...
- 验证：从中国新建 SSH OK、18080 healthz OK、wg-quick/zhhub/fail2ban 全 active、wg 3 个握手对端。
...
- 重启前确认（保命点 2 客户机）：wg-quick/zhhub/ufw/fail2ban/docker 均 enabled、
