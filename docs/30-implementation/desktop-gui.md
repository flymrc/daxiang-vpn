...
- 一键换 IP（手机卡出口）。
- 连接成功后自动设置 Windows 系统代理，浏览器和遵循系统代理的 App 无需手动配置。
...
- **当前交付 = 用户态代理 + GUI 自动设 Windows 系统代理**：连接时由 Rust 写 WinINET（注册表 `Internet Settings` 的 `ProxyEnable`/`ProxyServer` + `InternetSetOption` 刷新），断开 / 退出 / 崩溃时还原。**免管理员、无 UAC，对浏览器和大多数 App 即开即用。**
- **`--fast` 不再作为 GUI「全局模式」暴露**：现有 sing-box/WireGuard 配置只覆盖 Hub/内网段，并不是完整 `0.0.0.0/0` TUN 接管。后续真正全局 TUN 需要单独补 tun inbound、auto_route、DNS/IPv6/断开恢复验证。
...
| `connect(fast)` | `start [--fast]`；连接成功后统一由 `sysproxy.rs` 设 Windows 系统代理 |
...
- 产品显示名「纵横 VPN」（`productName`），二进制名 `zhvpn-desktop`（`mainBinaryName`，ASCII 避坑）；`publisher`/`copyright`/描述齐全；图标为自绘品牌图（蓝底白「象」，`tauri icon` 生成全套）。代码签名后续再补。
...
- **M3 已完成**（2026-06-09，代码层，待人工点验）：
  - 系统代理：`src-tauri/src/sysproxy.rs`（`sysproxy` crate）。`connect` 成功后自动把 Windows 系统代理指向本地代理；`disconnect` 还原。首次打开把原状态写 `proxy-backup.json`，还原后删除——**崩溃也能恢复**：启动时若备份存在且代理实际未运行则自动还原。
  ...
  - 2026-06-11 起界面不再暴露旧「全局模式 / 系统 TUN」复选框；连接后统一设置 Windows 系统代理，`--fast` 仅保留在命令层做后续实验入口。
