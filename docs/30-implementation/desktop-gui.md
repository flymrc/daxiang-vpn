...
- **全局代理选项 = 用户态代理 + GUI 自动设 Windows 系统代理**：用户勾选后，连接时由 Rust 写 WinINET（注册表 `Internet Settings` 的 `ProxyEnable`/`ProxyServer` + `InternetSetOption` 刷新），断开 / 退出 / 崩溃时还原。**免管理员、无 UAC，对浏览器和大多数 App 即开即用。**
...
| `connect(globalProxy)` | `start`；`globalProxy=true` 时连接成功后由 `sysproxy.rs` 设 Windows 系统代理 |
...
- **M3 已完成**（2026-06-09，代码层，待人工点验）：
  - 系统代理：`src-tauri/src/sysproxy.rs`（`sysproxy` crate）。勾选「全局代理」后，`connect` 成功会自动把 Windows 系统代理指向本地代理；默认局部代理模式不改系统代理。`disconnect` 还原。首次打开把原状态写 `proxy-backup.json`，还原后删除——**崩溃也能恢复**：启动时若备份存在且代理实际未运行则自动还原。
  ...
  - 2026-06-11 起界面不再暴露旧「全局模式 / 系统 TUN」复选框；改为「全局代理」复选框，默认局部代理，勾选后设置 Windows 系统代理。`--fast` 仅保留在 CLI 命令层做后续实验入口。
