...
- 出口 IP 分开展示 IPv6 / IPv4，避免把手机 IPv6 主路径与 v4-only Hub 兜底混在一起。
- 一键换 IP（手机卡出口）。
...
1. `status --json`：
   ...
   {"running":true,"proxy":"127.0.0.1:7890","proxy_reachable":true,
    "egress":"日本住宅","egress_ip":"240b:...","egress_ipv6":"240b:...","egress_ipv4":"36.50.84.68"}
   ```
   （未连接 / 无配置时为 `{"running":false,"proxy_reachable":false,"error":"…"}` 并退出非 0。
   当前不带 `fast` 字段——运行模式没有持久化存储，GUI 自己知道是以哪种模式拉起的。）
   `status --json --no-ip-check` 只返回本地运行状态、代理地址和出口名，不访问外部公网 IP 服务，供 GUI 高频轮询使用。
...
- **0.4.4 状态展示修正已完成**（2026-06-11）：
  ...
  - 前端分开展示「出口 IPv6 / 出口 IPv4」，并保留上一轮有效值，避免无实质换 IP 时 UI 反复跳「获取中」。
