...
- `/admin/api/tokens` 会额外返回可为空的 `last_active_at`,来源于当前活跃 token lease 的最近心跳时间,用于控制台排序展示,不包含 token 明文。
...
授权码页默认按 10 条/页分页,并按最近活跃倒序展示。授权码、客户端、状态、出口、WG 地址、到期、最近活跃表头支持点击排序;排序发生在分页前,切换排序会回到第 1 页。授权码列使用固定文本列 + 固定图标列布局,保证脱敏值和 reveal 后完整值的眼睛按钮纵向对齐。
