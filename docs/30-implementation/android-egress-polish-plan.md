...
| 12 | 住宅 WiFi 承载隧道腿 + 蜂窝 IPv6 出口 POC | ~半天 | 11 | 🧪 POC 进行中 |
| 13 | 审计修复: striped 取消路径 + debug 权限收窄 | ~45min | 10/11 | ✅ 已部署 2026-06-14 |
...
## 12. 住宅 WiFi 承载隧道腿 + 蜂窝 IPv6 出口 POC 🧪 POC 进行中

**目标**：把 Android -> Hub 的 `zhreverse` 反向隧道放到住宅 WiFi/家宽 IPv4 上,但 Android -> 目标网站仍走手机蜂窝 IPv6。若成功,可同时绕开 Rakuten IPv4/CGNAT/F5 的长连接问题,并保留目标网站看到手机 `240b:...` IPv6 出口。

目标数据流:

```text
zhreverse tunnel socket -> wlan0 / home WiFi IPv4 -> home relay -> Hub 36.50.84.68:39093
zhreverse target socket -> rmnet_data* / cellular IPv6 -> target website
```

**理论依据**：root Android 底层是 Linux,可以通过 `SO_BINDTODEVICE`、policy routing、fwmark 或 Android netd network handle 将不同 socket 绑到不同网络。难点不是 TCP 转发,而是 Android 默认 WiFi 优先路由会把目标网站 socket 也抢到 WiFi,必须显式分流。

**2026-06-14 WiFi 接入后只读确认**：

- `wlan0` 已连接住宅 WiFi,地址为 `192.168.3.3/24`,同时有 `2400:2412:...` 住宅 IPv6。
- 蜂窝数据网仍在线,`rmnet1` 有 `10.1.7.154/32` 和 `240b:c010:...` IPv6。
- Android 当前默认网络是 WiFi,WireGuard VPN 的 underlying network 也是 WiFi。
- Hub `/debug/session-health` 已看到 reverse tunnel 的 `remote_addr` 变成住宅公网 IPv4 `60.124.42.38:*`,说明隧道腿已随默认网络迁到 WiFi。
- 但经代理访问 `https://api6.ipify.org` 返回的是住宅 WiFi IPv6 `2400:2412:...`,不是蜂窝 `240b:c010:...`;目标网站 socket 也被 WiFi 默认路由抢走。
- `ip -6 route get ... oif rmnet1` 能查到蜂窝 IPv6 出路,说明底层具备分流条件。

**实现中的最小代码 POC**：

- Android client 新增 `tunnel_bind_interface` 与 `target_bind_interface` 两个可选配置字段。
- Linux 下通过 `SO_BINDTODEVICE` 绑定 socket;非 Linux 构建保留默认行为并在显式启用时返回不支持。
- `tunnel_bind_interface` 只支持 TCP/yamux 隧道;QUIC 隧道暂不纳入本轮 POC。
- 目标站点 TCP 拨号和目标 DNS 解析都跟随 `target_bind_interface`,用于测试 `target_bind_interface: rmnet1` 是否能保住蜂窝 IPv6 出口。

**2026-06-14 部署与结论**：

- 已部署到 Android,二进制 SHA256 `7d813b5d2f42548e4210ce3c6595034b071cccebfe23e4b9d4d5e80e55e990af`。
- 备份:`/data/adb/zhreverse/bin/zhreverse.bak-20260614-bind-iface-poc`、`/data/adb/zhreverse/client.yaml.bak-20260614-bind-iface-poc`。
- 当前 `client.yaml` 已启用:
...
1. 手机开启 WiFi + 蜂窝,并打开“移动数据始终开启”。只读确认蜂窝 IPv6 仍存在:
...
2. 在住宅 WiFi 局域网放一个 relay,例如 `192.168.1.x:39093 -> 36.50.84.68:39093`。可先用 `socat` / `rinetd` / 路由器端口转发做最小验证。

3. 手机临时跑第二个 `zhreverse client` 或临时改 `server` 指向 `192.168.1.x:39093`,不要先改生产 service。先确认 Hub `/debug/session-health` 的 `remote_addr` 变成住宅公网 IPv4。

4. 验证目标出口是否仍是手机蜂窝 IPv6:

   ```bash
   curl -x http://10.66.0.1:18081 -s https://api6.ipify.org; echo
   ```

   成功标准:返回仍是 `240b:...`,不是住宅 WiFi 出口。

5. 如果目标网站被 WiFi 抢走,再进入代码/系统分流:

   - `tunnel_bind_interface: wlan0`
   - `target_bind_interface: rmnet_dataX`
   - 必要时加 `target_bind_mark` / Android netd network handle
   - DNS 也优先走蜂窝侧,避免 CDN 选择偏 WiFi。

**成功判据**：

- Hub `session-health.sessions[].remote_addr` = 住宅公网 IPv4。
- 经代理访问 `api6.ipify.org` = 手机蜂窝 `240b:...` IPv6。
...
## 13. 审计修复: striped 取消路径 + debug 权限收窄 ✅ 已部署 2026-06-14
...
**部署**：Hub 已部署新二进制并在 `/etc/zongheng/zhreverse/server.yaml` 增加:
...
Android 二进制无需为该修复变更,未部署 Android。部署信息:

- Hub SHA256 `91dbae431dece50d8e034b369cd936a388e1a19a8b716097832773fe23b24ef9`,备份 `/opt/zongheng/zhreverse/zhreverse.bak-20260614-audit-hardening`。
- Hub 配置备份 `/etc/zongheng/zhreverse/server.yaml.bak-20260614-audit-hardening`。
- 部署后 `zhreverse-hub.service` active,`/debug/session-health` 和 1KB `/debug/tunnel-bench` 从 Hub 本机可用。
- `check-android-egress-health.ps1` PASS,两条 Android reverse session 在线且 `consecutive_failures=0`。
