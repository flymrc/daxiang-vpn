...
| 控制 CLI | `dxvpn` | 导入配置、启动本地代理、同步配置、检查健康状态、切换出口 |
| 桌面 GUI | `大象 VPN`（Tauri，Windows） | 面向终端用户的图形客户端：把 `dxvpn` 作为 sidecar 调用（登录/连接/换 IP/状态），用户态模式下自动设/还原 Windows 系统代理。详见 [桌面 GUI 实现方案](../30-implementation/desktop-gui.md) |
...
Hub 通常不应该作为最终公网出口。Hub 的职责是中转中国客户端和日本出口节点之间的 WireGuard 流量。
...
WireGuard App 仍负责内网控制面,例如 `10.66.0.101:2022` SSH 运维和 watchdog 自愈。Android 客户端 token 的 `egress.proxy_addr` 应指向 Hub 的 WireGuard 地址 `10.66.0.1:18081`,不要再指向手机旧入站代理。旧 `dxandroid-egress` / `10.66.0.101:1080` Android 数据面已从生产入口拆除。
...
- 手动分配：`dxvpn client assign --client cn-laptop --egress jp-mac-01`
...
dxvpn hub status
dxvpn hub diagnostics

dxvpn peer list
dxvpn peer add --name jp-mac-01 --role egress --ip 10.66.0.100 --public-key ...
dxvpn peer remove --name jp-mac-01

dxvpn egress list
dxvpn egress assign --client cn-windows-01 --egress jp-mac-01
dxvpn egress health --name jp-mac-01

dxvpn client config --name cn-windows-01
dxvpn client route --name cn-windows-01 --mode full --egress jp-mac-01
dxvpn client route --name cn-windows-01 --mode split --cidr 1.2.3.0/24 --egress jp-mac-01
