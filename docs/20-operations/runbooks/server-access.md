...
- 数据面：`zhreverse` 反向 TCP/yamux（2026-06-14 双网络 POC:隧道绑 `wlan0`,目标 TCP/DNS 绑 `rmnet1`）
- Hub 侧代理入口：`10.66.0.1:18081`
- Hub reverse TCP 监听：`0.0.0.0:39093/tcp`
- Android reverse endpoint：Android 主动连 Hub,无手机入站端口;当前 POC 下本机 socket 为 `192.168.3.3%wlan0 -> 36.50.84.68:39093`
...
- Android 当前双网络 POC 配置:`tunnel_bind_interface: wlan0`,`target_bind_interface: rmnet1`。Hub `/debug/session-health` 应看到住宅公网 IPv4 `60.124.42.38:*`;经代理访问 `https://api6.ipify.org` 应回手机蜂窝 `240b:c010:...`。若 WiFi/家宽断开,该 POC 不会自动 fallback 到蜂窝隧道;回滚为恢复 `/data/adb/zhreverse/client.yaml.bak-20260614-bind-iface-poc` 或移除 `tunnel_bind_interface` / `target_bind_interface`,再 `pkill zhreverse`。
