...
第一次修复先把根路径改成 302 到 `/admin/`,随后按产品要求改为根路径直接 404,避免裸域名暴露控制台入口提示。

最终修复:

- 备份 `/etc/caddy/Caddyfile` 到 `/etc/caddy/Caddyfile.bak-20260701130056-root-route-redir-fix`。
- 再备份 `/etc/caddy/Caddyfile` 到 `/etc/caddy/Caddyfile.bak-20260701130311-root-404`。
- 移除根路径跳转;fallback 为 `handle { respond "not found" 404 }`。
- `caddy validate --config /etc/caddy/Caddyfile` 通过。
- `systemctl reload caddy` 成功,Caddy 保持 active。

验证:

- `https://jp-proxy.ruichao.dev/` 返回 `404 Not Found`。
