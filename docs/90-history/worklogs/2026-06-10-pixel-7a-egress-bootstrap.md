...
- 部署 `dxreverse-linux-arm64` 到 `/data/adb/dxreverse/bin/dxreverse`。
- 复制 Hub `/etc/daxiang/dxreverse/token` 到 Pixel `/data/adb/dxreverse/token`。
- 部署 `/data/adb/dxreverse/client.yaml` 与 `/data/adb/service.d/99-dxreverse-egress.sh`。
- 手动拉起 `99-dxreverse-egress.sh` supervisor。
...
- Pixel 控制面 WireGuard / `dxandroid-control` 尚未迁移,因此 `10.66.0.101:2022` 暂时不可作为 Pixel 控制面使用。
...
Pixel 7a 数据面可以接入 `dxreverse`,但在当前位置/当前 Rakuten 小区下,HTTPS 大下载仍明显不稳定,尚未看到相对 Motorola 的确定改善。`connections: 2` 在 Pixel 上第二条 session 多次 `i/o timeout`,当前保留 `connections: 1` 与历史最稳组合一致。

后续若继续推进 Pixel 替换,需要补齐 WireGuard 控制面和 `dxandroid-control` 自愈,再做换位置/换小区/换 SIM 的对比测速。
