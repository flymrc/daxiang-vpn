...
- **打包配置**（`tauri.conf.json`）：
  - `productName` 改为「纵横 VPN」（显示名），新增 `mainBinaryName: zhvpn-desktop`（ASCII 二进制名，避免 unicode 坑）。
...
2. unicode `productName`「纵横 VPN」让 WiX `light.exe`（MSI）失败。NSIS 处理 UTF-8 正常。→ 去掉 MSI，只出 NSIS（消费者首选安装器，MSI 偏企业 GPO）。

## 验证

- `tauri build` 通过，产出 `src-tauri/target/release/bundle/nsis/纵横 VPN_0.1.0_x64-setup.exe`（7.5MB）。
- sidecar 确认随包：`target/release/zhvpn.exe`（17.2MB，由 `zhvpn-x86_64-pc-windows-msvc.exe` 去 triple 后缀复制而来）。
- 主程序 `zhvpn-desktop.exe` 11.2MB，嵌入品牌图标。
...
附：构建产物只应有单一 `纵横 VPN_…setup.exe`（含内置 CLI sidecar，NSIS LZMA 压缩约 7.5MB）；调试期改名/改格式残留的旧包（`Zongheng VPN…msi/exe`）已清理。沿用 `cargo build`（非 `tauri build`）跑出的二进制会指向 dev server，只能 `npm run tauri dev` 或装/跑 `tauri build` 产物。
